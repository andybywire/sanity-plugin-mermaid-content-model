...
- **Re-verify the access (and the `_original` shape) whenever widening the `sanity` peer range.** Done for v6 in [#3](https://github.com/andybywire/sanity-plugin-mermaid-content-model/issues/3): the published peer range is now `^5 || ^6`, reconciling the prior gap (peer `^5` vs. a bundled dev studio on `^6`). Verified at the **type and install** level — `tsc` / `pkg-utils build` against Studio 6's types, plus a clean strict-peer install into both a Sanity 5 and a Sanity 6 studio ([`scripts/test-studio-install.mjs`](../../scripts/test-studio-install.mjs)). The **runtime** half — that `_original.types` still carries raw `defineType`s with their `validation` **functions** intact — is now exercised automatically by the **archetype-schema golden tests** ([#19](https://github.com/andybywire/sanity-plugin-mermaid-content-model/issues/19)): they compile real schemas through `createSchema` and read them back via `readSchemaSource`, so a broken seam fails CI rather than degrading silently. (A visual render against a real Studio is still a useful release eyeball, but the structural re-verify no longer depends on it.) The same coverage gates any future major (v7+).

  > **Superseded in part by [ADR 0004](0004-sanity-ui-v4-support-boundary.md).** The `^5 || ^6` range described here was narrowed to `^6.9.2` for 2.0, when `@sanity/ui` moved to a v4 peer. The re-verification rule still holds and now applies when the range moves in **either** direction — narrowing changes which release the seam is actually exercised against.
