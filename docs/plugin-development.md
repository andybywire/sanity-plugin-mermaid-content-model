...
- **The bundled `studio/` carries a synthetic schema** (`studio/schemaTypes.ts`) whose job is to exercise every shape the diagram cares about — documents, named/inline objects, references, images, the Portable Text variants, an orphan, validation. A change that introduces a **new diagram shape** should add a representative case there so it can be eyeballed (`pnpm dev` → Content Model).
- **The gate's `pnpm typecheck` covers `src/` only — `studio/` is _not_ checked** (it has its own `tsconfig.json`). Verify studio edits (e.g. the synthetic schema) with `tsc -p studio/tsconfig.json` or `pnpm dev` + eyeball. Expect a _pre-existing_ type error in `studio/sanity.config.ts` from the dual `@sanity/types` versions in the store (the plugin peers `sanity@^5`, the studio runs `@^6` — the planned v1.0 peer-range pivot, see [ADR 0002](decisions/0002-content-model-plugin-architecture.md)); it's unrelated to your edit — don't chase it.
