...
- **A green release run does not mean the package is installable yet — npm publishes asynchronously.** On the trusted-publishing path npm accepts the upload and queues it (`npm notice Your package is being processed and may take a few minutes to become available`), so semantic-release logs `Published <pkg>@<version>` and the job goes green while the registry still serves the previous version. On 2.0.0 the job reported success at `00:15:50Z` and the version appeared on the registry at `00:17:56Z` — about two minutes. Don't read a missing version straight after the run as a failed publish; that mistake was made here, and the log line above is what settles it.

  The **npm website lags further still**, and renders inconsistently while it catches up: the header and Current Tags can show the old version while Version History already lists the new one as Published with no publish date and no provenance badge. That is a stale page, not a broken release. Check the registry, which is authoritative:

  ```bash
  curl -s https://registry.npmjs.org/<pkg> | jq '{latest: .["dist-tags"].latest, published: .time["<version>"]}'
  npm view <pkg>@<version> dist.attestations   # provenance is attached, or it isn't
  ```

  A genuine provenance failure looks different from a slow one: the version exists in the registry JSON with **no `dist.attestations`**. If it's there with a `slsa.dev/provenance/v1` predicate, the badge is only a matter of time.
